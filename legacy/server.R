source(file = "legacy/librarys.R",encoding = "UTF-8",local = F)
source(file = "legacy/panel.R",encoding = "UTF-8",local = F)
#source(file = "source.sql.R",encoding = "UTF-8",local = F)

source(file = "legacy/funcoes.R",encoding = "UTF-8",local = F)
...
source(file = "legacy/source.descritivo2.R",encoding = "UTF-8",local = F)


#source(file = "aba_map_server.R",encoding = "UTF-8",local = F)
#source(file = "source.maps.density.R",encoding = "UTF-8",local = F)

lia                 <- readxl::read_xls(path = "data/raw/Planilha IIP.xls",sheet = "Plan1")
...
regionais                 <- readxl::read_xlsx(path = "data/raw/regionais.xlsx",sheet = "Planilha1")
coordenadas.municipios    <- readxl::read_xls(path = "data/raw/Coordenadas_Municipios.xls",sheet = "Cidades")  %>% rename(Codigo=GEOCODIGO_MUNICIPIO)
regionais$nome            <- tolower(rm_accent(as.character(regionais$nome)))

maps.cities2               <- get_brmap(level = "municipality", filters = list(state_code = 41), output = "sf") %>%
   select(name, municipality_code) %>%
   rename(nome = name, Codigo = municipality_code)
...
size_card = c(8,4)
 ...
 server <- function(input, output) {
    ...
    dados.maps.pr                       <- readxl::read_xlsx(path = "data/raw/dados.pr.xlsx",sheet = "Worksheet") %>% select(`Município [-]`,`Código [-]`,`População estimada - pessoas [2019]`,`Área Territorial - km² [2019]`) %>% rename(Codigo=`Código [-]`)
   ...
   observeEvent(input$form_generate, {
     ...
     #  \\end{titlepage}\n")
      ...
      tryCatch({
         ...
         showNotification("FORMULÁRIO GERADO COM SUCESSO!!!", type = "message")
         output$relatorio <- renderUI({
            tags$iframe(src = paste0("pdf_folder/formulario.pdf?version=", as.integer(Sys.time())),
                        align = "left", width = 800, height = 600,
                        style = "overflow-x:hidden;overflow-y:hidden;border-width:10px;")
         })
      }, error = function(error) {
         showNotification(paste("Erro ao gerar o relatório:", conditionMessage(error)),
                          type = "error", duration = NULL)
         output$relatorio <- renderUI({
            tags$p("Não foi possível gerar o relatório em PDF.", class = "text-danger")
         })
      })
   })
